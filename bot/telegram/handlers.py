...
from data.client import GoMarketREST,MockGoMarketREST
...
async def start(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("GoQuant FinBot ready. Use /help")


gm_rest = MockGoMarketREST(settings.gomarket_api_key) if settings.mock else GoMarketREST(settings.gomarket_api_key)

async def list_symbols(update: Update, _: ContextTypes.DEFAULT_TYPE):
    exchange = _.args[0].lower()
    async with gm_rest as client:
        syms = await client.list_symbols(exchange)
    await update.message.reply_text(", ".join(syms[:20]))    
